...
const RadioGroups = props => {
  ...
  props.items.forEach(item => {
    ...
    // Escapes for special ship equipment
    if (item.id === 'engine_pir_xl_battleship_01_allround_01_mk1') {
      base = 'XL All-round Engine';
      race = 'ARG';
      item.name = 'Erkling*';
    } else if (item.id === 'shield_pir_xl_battleship_01_standard_01_mk1') {
      base = 'XL Shield Generator';
      race = 'ARG';
      item.name = 'Erkling*';
    } else if (item.id === 'engine_gen_m_yacht_01_mk1') {
      base = 'M All-round Engine';
      race = 'ARG';
      item.name = 'Astrid*';
    }
...
const Ships = (props) => {
  ...
  useEffect(() => {
    ...
    if (props.x4.ships) {
      ...
      Object.keys(ships)
        .forEach(key => (availableTypes.indexOf(ships[key].type) === -1 && availableTypes.push(ships[key].type)));
      setTypes(availableTypes);

      const allShips = Object.keys(props.x4.ships).map(size => Object.keys(props.x4.ships[size])
        .map(ship => props.x4.ships[size][ship].name));
      seo({
