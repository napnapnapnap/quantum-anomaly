...
import { backgroundLabelRectWidth, getHexagonPointsV2, getHexagonPointsV3, maps } from "../helpers";
          ...
          <div ref={wrapper} className="x4__map-wrapper">
            ...
            <svg ref={svg} width={width} height="100%" viewBox="-2800 -1800 5300 5000"
              ...
              {props.x4.map.clusters.map(cluster => {
                ...
                return <polyline key={cluster.name}
                                 fill="none"
                                 stroke={color}
                                 strokeWidth="2"
              ...
              {props.x4.map.clusters.map(cluster => cluster.sectors.map(sector => sector.zones.map(zone =>
                zone.gates.map((gate, index) =>
                  <React.Fragment key={`${zone.zoneReference}-${index}`}>
                    <circle cx={gate.position.x}
                            cy={-gate.position.z}
                            fill="gray"
                            stroke="white"
                            strokeWidth="1"
              ...
              {Object.keys(props.x4.map.gates).map(key => (
                props.x4.map.gates[key].end && <React.Fragment key={key}>
                  <line x1={props.x4.map.gates[key].start.x} y1={-props.x4.map.gates[key].start.z}
                        x2={props.x4.map.gates[key].end.x} y2={-props.x4.map.gates[key].end.z} stroke="black"
                        strokeWidth="5" />
                  <line x1={props.x4.map.gates[key].start.x} y1={-props.x4.map.gates[key].start.z}
                        x2={props.x4.map.gates[key].end.x} y2={-props.x4.map.gates[key].end.z} stroke="white"
              ...
              {props.x4.map.sectorHighways.map(highway => (
                <React.Fragment key={Math.random()}>
                  <circle cx={highway.origin.x} cy={highway.origin.y} fill="blue" stroke="white" strokeWidth="1"
                          r="3" />
