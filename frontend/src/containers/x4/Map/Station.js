...
const Station = props => {
  if (props.tags !== "tradestation" && props.tags !== "equipmentdock" && props.tags !== "wharf" && props.tags !== "shipyard")
    return <></>;

  const scale = `scale(${props.stationScale})`;
  const translateX = parseInt((props.position.x - 8 * props.stationScale) / props.stationScale, 10);
  const translateY = parseInt((-props.position.z - 8 * props.stationScale) / props.stationScale, 10);
