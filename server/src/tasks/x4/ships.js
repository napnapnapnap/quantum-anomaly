...
async function processShips(
  ...
  const macroData = await parser.parseStringPromise(
    await fs.readFile(macroPath)
  );

  let ship = addDataFromMacroFile(
    macroData,
    translations,
    defaults,
    storage,
  ...
  dataPath = `${dataPath
    .splice(0, dataPath.length - 2)
    .join("\\")}\\${dataFile}.xml`;
  if (
    dataPath.indexOf("ego_dlc_split") !== -1 &&
    dataPath.indexOf("ship_arg_xl_builder_01") !== -1
  ) {
    dataPath = dataPath.replace("extensions\\ego_dlc_split\\", "");
  }
  if (
    dataPath.indexOf("ego_dlc_terran") !== -1 &&
    dataPath.indexOf("ship_arg_xl_builder_01") !== -1
  ) {
    dataPath = dataPath.replace("extensions\\ego_dlc_terran\\", "");
  }
  if (
    dataPath.indexOf("ego_dlc_pirate") !== -1 &&
    dataPath.indexOf("ship_arg_xl_builder_01") !== -1
  ) {
...
export async function getShips(
  ...
  await shipFileList.reduce(async (prev, shipFile) => {
    ...
    if (shipFile.indexOf("ship_tfm_l_carrier_01_a_macro") !== -1)
      return Promise.resolve();
    if (shipFile.indexOf("ship_gen_s_lasertower_01_a_macro") !== -1)
      return Promise.resolve();
    if (shipFile.indexOf("ship_spl_s_trans_container_01_plot_01_macro") !== -1)
      return Promise.resolve();
    if (shipFile.indexOf("ship_spl_m_bomber_01_a_macro") !== -1)
      return Promise.resolve();
    if (shipFile.indexOf("ship_spl_xl_battleship_01_a_macro") !== -1)
      return Promise.resolve();
    if (shipFile.indexOf("ship_kha_s_fighter_01_a_macro") !== -1)
      return Promise.resolve();
    if (shipFile.indexOf("ship_gen_m_transdrone_container_01_a_macro") !== -1)
      return Promise.resolve();
    if (shipFile.indexOf("ship_gen_m_transdrone_container_02_a_macro") !== -1)
      return Promise.resolve();
    if (shipFile.indexOf("ship_gen_s_transdrone_container_01_a_macro") !== -1)
      return Promise.resolve();
    if (shipFile.indexOf("ship_gen_s_transdrone_container_02_a_macro") !== -1)
      return Promise.resolve();
    if (shipFile.indexOf("ship_ter_l_trans_container_01_landmark_macro") !== -1)
      return Promise.resolve();
    if (shipFile.indexOf("ship_ter_l_research_01_a_macro") !== -1)
      return Promise.resolve();
    ...
    if (
      shipFile.indexOf("ship_pir_l_scavenger_01_a_storyhighcapacity_macro") !==
      -1
    )
      return Promise.resolve();

    const ship = await processShips(
      shipFile,
      translations,
      defaults,
      equipment.storage,
      equipment.shipstorage,
      wares
    );
    ships[ship.id] = ship;
  }, Promise.resolve());
