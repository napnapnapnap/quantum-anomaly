                  ...
                  mapDefaults.forEach((mapDefault) => {
                    ...
                    if (sector.name === mapDefault.macro) {
                      ...
                      sector.description = translateRecursiveTrim(
                        mapDefault.properties.identification.description,
                        translations
                      );
                      ...
                      zones.forEach((zone) => {
                        if (sectorConnection.macro.ref === zone.name) {
                          ...
                          if (zone.connections && typeof zone.connections !== 'string') {
                            if (!Array.isArray(zone.connections.connection))
                              zone.connections.connection = [zone.connections.connection];
                ...
                regionDefinitions.forEach((regionDefinitionItem) => {
                  if (clusterConnection.macro.properties.region.ref === regionDefinitionItem.name) {
                    ...
                    if (
                      ...
                      appWarning(
                        `Correcting Cluster108_Sector002_Region001_macro to Cluster110_Sector001_Region001_macro`
                      );
                    }
                    if (
                      ...
                      appWarning(
                        `Correcting Cluster104_Sector001_Region002_macro to Cluster112_Sector002_Region002_macro`
                      );
                    }
                    ...
                    if (regionDefinition.miningRegionVolume > 200 * 200 * 100)
                      regionDefinition.miningRegionVolume = 200 * 200 * 100;
                    ...
                    if (regionDefinition.fields.nebula) {
                      if (!Array.isArray(regionDefinition.fields.nebula))
                      ...
                      if (regionDefinition.fields.asteroid)
                        regionDefinition.fields.asteroid.forEach((asteroid) => {
                          if (asteroid.resource === resource.ware) {
                            const asteroidDensityFactor =
                              parseFloat(asteroid.densityfactor) > 50 ? 3 : parseFloat(asteroid.densityfactor);
                            const asteroidMinMaxNoise =
                              (parseFloat(asteroid.minnoisevalue) + parseFloat(asteroid.maxnoisevalue)) / 2;
                            const asteroidYield = parseFloat(asteroid.yield);
                            const asteroidYieldVariation = parseFloat(asteroid.yieldvariation);
                            const asteroidProps =
                              asteroidDensityFactor * asteroidMinMaxNoise * asteroidYield * asteroidYieldVariation;
        ...
        if (sector.name === sectorName) {
          ...
          Object.keys(region.resourceQuantities).forEach((resource) => {
            ...
            if (resource === 'ore' || resource === 'silicon') {
              ...
              if (sector.resourcesTotal[resource] > HYDROGEN_HELIUM_CAP)
                sector.resourcesTotal[resource] = HYDROGEN_HELIUM_CAP;
