...
export default async function () {
  ...
  let npcs = types.map(npc => {
    if (npc.dogma_attributes) npc.dogma_attributes = npc.dogma_attributes.map(attr => {
      let fullAttribute = dogmaAttributes.find((dogmaAttr => dogmaAttr.id === attr.attribute_id));
      return {
        name:  fullAttribute.data.display_name.length !== 0 ? fullAttribute.data.display_name : fullAttribute.data.name,
        value: attr.value
      };
    });

    if (npc.dogma_effects) npc.dogma_effects = npc.dogma_effects
      .map(effect => EsiDogmaEffects.find((dogmaEffect => dogmaEffect.id === effect.effect_id)).data);
